...
namespace Save_Coord_RAGE.CoordinateManager
{
    ...
    {
        /* Thanks to Albo1125 for his open source code
        ...
        internal static string GetLowerZoneNameForEntity(this Entity entity)
        {
            return GetZoneName(GetZone(entity.Position)).ToLower();
        }
        internal static string GetLowerZoneName(Vector3 pos)
        {
        ...
        internal static EWorldZone GetZone(Vector3 Position)
        {
        ...
        "tataviam mountains", "noose hq", "palomino highlands", "davis"};

        internal enum WorldDistricts { City, LosSantosCountryside, BlaineCounty, Water }
        internal static WorldDistricts GetWorldDistrict(this Vector3 pos)
        {
            if (pos.IsPointOnWater()) { return WorldDistricts.Water; }
            uint zoneHash = NativeFunction.CallByHash<uint>(0x7ee64d51e8498728, pos.X, pos.Y, pos.Z);
        ...
        internal static bool IsPointOnWater(this Vector3 position)
        {

            return NativeFunction.Natives.GET_WATER_HEIGHT<bool>(position.X, position.Y, position.Z, out float height);

        }
        internal static string GetZoneName(EWorldZone zone)
        {
            switch (zone)
            {
